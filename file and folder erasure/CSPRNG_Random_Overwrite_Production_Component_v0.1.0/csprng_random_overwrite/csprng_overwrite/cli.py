...
import sys
from .engine import overwrite_file, overwrite_tree, write_audit, CSPRNGOverwriteError
...
def main(argv=None) -> int:
    ...
    p.add_argument("--confirm", help="Must exactly match target when --execute is used.")
    p.add_argument("--verify", action="store_true", help="Verify post-overwrite content differs from original SHA-256.")
    p.add_argument("--keep", action="store_true", help="Keep the overwritten file instead of unlinking it.")
    ...
    try:
        if target.is_dir():
            results = overwrite_tree(
                target, verify=args.verify, remove=not args.keep,
                chunk_size=args.chunk_size
            )
        else:
            results = [overwrite_file(
                target, verify=args.verify, remove=not args.keep,
                chunk_size=args.chunk_size
            )]
