import json
import pytest
from slack_sanitizer.engine import calculate_tail, sanitize_tail, SlackError, SyntheticBackend, write_audit
...
def test_random_tail_sanitization(tmp_path):
    p=tmp_path/"file"; p.write_bytes(b"A"*5000)
    r=sanitize_tail(p,backend=SyntheticBackend(4096),pattern="random",verify=True)
