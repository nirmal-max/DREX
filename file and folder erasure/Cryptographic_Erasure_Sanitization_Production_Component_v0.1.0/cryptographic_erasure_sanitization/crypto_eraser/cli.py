...
def main(argv=None):
    ...
    d=sub.add_parser("destroy-key"); d.add_argument("key_id"); d.add_argument("--store",default=".crypto-keys"); d.add_argument("--execute",action="store_true"); d.add_argument("--confirm"); d.add_argument("--target-id"); d.add_argument("--audit")
    ...
    if a.cmd=="create-key":
        ks.create_key(a.key_id); print(json.dumps({"status":"CREATED","key_id":a.key_id})); return 0

    if a.cmd=="create-secret":
        if not ks.exists(a.key_id): ks.create_key(a.key_id)
        env=create_envelope(ks,a.key_id,a.text.encode(),a.output)
        save_envelope(a.output,env)
    ...
    if a.cmd=="destroy-key":
        if not a.execute:
            print(json.dumps({"mode":"dry-run","action":"destroy-key","key_id":a.key_id,"warning":"No key was destroyed."},indent=2))
            return 0
        ...
        try:
            ...
            print(json.dumps({"status":"SANITIZED","key_id":a.key_id,"target_id":target},indent=2))
            return 0
