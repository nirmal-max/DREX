...
bool acquire_file(const std::string&src,const std::string&dst,std::uint64_t&size,std::string&e){
 ...
 char buf[1024*1024];size=0;
 while(in){in.read(buf,sizeof(buf));auto n=in.gcount();if(n>0){out.write(buf,n);size+=(std::uint64_t)n;}if(in.bad()){e="source read error";return false;}}
 if(!out){e="evidence write error";return false;} return true;
}
