...
int main(int argc,char**argv){
 ...
 for(int i=1;i<argc;i++){std::string a=argv[i];if(a=="--source"&&i+1<argc)source=argv[++i];else if(a=="--types"&&i+1<argc)types=argv[++i];else if(a=="--rules"&&i+1<argc)rulesfile=argv[++i];else if(a=="--output"&&i+1<argc)outfile=argv[++i];else if(a=="--recover"&&i+1<argc)recover_id=std::stoll(argv[++i]);else if(a=="--destination"&&i+1<argc)dest=argv[++i];else if(a=="--list-rules"){for(auto&s:targeted::list_rule_ids())std::cout<<s<<"\n";return 0;}else {usage();return 2;}}
 ...
 auto result=targeted::scan(*src,rules);
 for(auto&c:result.candidates)targeted::validate_candidate(*src,c);
 if(!outfile.empty()){std::ofstream f(outfile);f<<targeted::to_json(result);}else std::cout<<targeted::to_json(result)<<"\n";
